...
void dijkstra (node *nd, Grafo *g, int **matrix){
  ...
  for (int src = 0; src < g->n; src++){
    ...
    int dist[g->n]; // Vetor de distâncias mais curtas
    int menorCaminho[g->n]; // Conjunto de vértices incluídos no caminho mais curto
    ...
    for (int j = 0; j < g->n - 1; j++) {
      int u = minDistance(dist, menorCaminho, g);

      menorCaminho[u] = 1;

      for (int v = 0; v < g->n; v++) {
        if (!menorCaminho[v] && matrix[u][v] && dist[u] != INFINITO &&
...
// Menu Options
void menu(node *nd, Grafo *g, int **matrix) {
  int op, vDfs, source, search, ret_bfs, v, s;
  ...
  while (op != 13) {
    ...
    case 10:
      // Bellman-Ford
      printf("\nBellman-Ford * * * * * * * * * * * * * \n");
      printf("Escolha o vertice de origem, para descobrir as distancias a "
             "partir dele: ");
      scanf(" %d", &v);
      printf("\n");
      bellmand_Ford(v, nd, matrix);
      printMatrixBF(v);

      /*printf("\n Utilizando o vetor de caminho minimo para descobrir se existe
      um caminho entre dois vertices!\n"); printf("Escolha o vertice de origem:
      "); scanf(" %d", &v); printf("\nEscolha o vertice que você deseja
      alcançar: "); scanf(" %d", &s); printf("\n"); printf("Caminho:\n");
      PrintCaminho(s, v);*/
      break;
...
int main() {
  ...
  //  Prints
  printf("\n  * * * * * * * * * * * * * * * * * * * * * * * \n");
  printf("* * * * * * Grafo como lista de adjacencia * * * * * * \n");
  printf("\n");
  print_graph_list(&g);
  printf("\n");
  printf("\n");
