...
int *A_Star(int start, int goal, int (*heuristicFunc)(int, int, int), int gridWidth, int (*distanceFunc)(int, int, int), Grafo *g) {
  int MAX_NODES = g->n;
  int *openSet = NULL;
  int openSetSize = 1;
...
int aEstrela(Grafo *g) {
  ...
  for (int i = 0; i < g->n; i++){
    for (int j = 0; j < g->n; j++){
      ...
      int *path = A_Star(startNode, goalNode, heuristic, gridWidth, distance, g);
